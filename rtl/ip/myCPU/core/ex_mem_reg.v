// ============================================================================
// ex_mem_reg — EX/MEM 流水线寄存器（valid/ready + stall + flush）
// ============================================================================
module ex_mem_reg(
    input  clk, input  reset,
    input         valid_in,  input  ready_in,
    input         flush,     input  stall,
    input  [31:0] alu_result_in, input [31:0] rs2_in, input [4:0] rd_in,
    input  mem_read_in,    input mem_write_in,  input mem_to_reg_in,
    input  reg_write_in,   input [1:0] mem_width_in, input mem_signed_in,
    input  csr_rd_in,      input csr_wr_in,     input csr_xchg_in,
    input  [13:0] csr_addr_in, input [31:0] csr_wdata_in,
    input  [31:0] csr_wmask_in,

    output        ready_out,
    output reg    valid_out,     output reg [31:0] alu_result_out,
    output reg [31:0] rs2_out,   output reg [4:0]  rd_out,
    ...
    output reg csr_rd_out,       output reg csr_wr_out,
    output reg csr_xchg_out,     output reg [13:0] csr_addr_out,
    ...
    always @(posedge clk) begin
        if (reset || flush) begin
            valid_out <= 0; alu_result_out <= 0; rs2_out <= 0; rd_out <= 0;
            mem_read_out <= 0; mem_write_out <= 0; mem_to_reg_out <= 0;
            reg_write_out <= 0; mem_width_out <= 2'b10; mem_signed_out <= 1'b1;
            ...
            valid_out <= valid_in;
            if (valid_in) begin
                alu_result_out <= alu_result_in; rs2_out <= rs2_in;
                rd_out <= rd_in;
