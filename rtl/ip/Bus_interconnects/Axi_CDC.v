...
module Axi_CDC (
  ...
  wire       [57:0]   cpu5_ar_push_payload;
  wire       [57:0]   cpu5_ar_pop_payload;
  wire       [39:0]   cpu5_r_push_payload;
  wire       [39:0]   cpu5_r_pop_payload;

  assign cpu5_ar_push_payload =
      {axiIn_arprot, axiIn_arcache, axiIn_arlock, axiIn_arburst,
       axiIn_arsize, axiIn_arlen, axiIn_arid, axiIn_araddr};
  assign {arFifo_io_pop_payload_prot, arFifo_io_pop_payload_cache,
          arFifo_io_pop_payload_lock, arFifo_io_pop_payload_burst,
          arFifo_io_pop_payload_size, arFifo_io_pop_payload_len,
          arFifo_io_pop_payload_id, arFifo_io_pop_payload_addr} =
      cpu5_ar_pop_payload;

  assign cpu5_r_push_payload =
      {axiOut_rlast, axiOut_rresp, axiOut_rid, axiOut_rdata};
  assign {rFifo_io_pop_payload_last, rFifo_io_pop_payload_resp,
          rFifo_io_pop_payload_id, rFifo_io_pop_payload_data} =
      cpu5_r_pop_payload;
  ...
  cpu5_async_fifo #(
    ...
    .wr_clk    (axiInClk),
    .wr_resetn (axiInRstn),
    .wr_valid  (axiIn_arvalid),
    .wr_ready  (arFifo_io_push_ready),
    .wr_data   (cpu5_ar_push_payload),
    .rd_clk    (axiOutClk),
    .rd_resetn (axiOutRstn),
    .rd_valid  (arFifo_io_pop_valid),
    .rd_ready  (axiOut_arready),
    .rd_data   (cpu5_ar_pop_payload)
  );
  cpu5_async_fifo #(
    .WIDTH      (40),
    .ADDR_WIDTH (3)
    ...
    .wr_clk    (axiOutClk),
    .wr_resetn (axiOutRstn),
    .wr_valid  (axiOut_rvalid),
    .wr_ready  (rFifo_io_push_ready),
    .wr_data   (cpu5_r_push_payload),
    .rd_clk    (axiInClk),
    .rd_resetn (axiInRstn),
    .rd_valid  (rFifo_io_pop_valid),
    .rd_ready  (axiIn_rready),
    .rd_data   (cpu5_r_pop_payload)
  );
...
// Compact first-word-fall-through asynchronous FIFO used on the latency
// critical AXI AR and R channels.  The Gray pointers still cross through two
// ASYNC_REG stages, while the small distributed RAM avoids the two additional
// registered pop stages in the generated StreamFifoCC implementation.
module cpu5_async_fifo #(
  parameter WIDTH = 32,
  ...
  wire [PTR_WIDTH-1:0] wr_ptr_plus_one = wr_ptr_bin + 1'b1;
  wire [PTR_WIDTH-1:0] wr_gray_plus_one =
      (wr_ptr_plus_one >> 1) ^ wr_ptr_plus_one;
  ...
  wire [PTR_WIDTH-1:0] wr_ptr_next = wr_ptr_bin + wr_fire;
  wire [PTR_WIDTH-1:0] rd_ptr_next = rd_ptr_bin + rd_fire;

  assign wr_ready = (wr_gray_plus_one != full_compare);
  assign rd_valid = (rd_ptr_gray != wr_gray_sync2);
