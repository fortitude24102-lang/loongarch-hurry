...
module axi2sram_sp_external #(
    ...
    always @ (*) begin
        ...
        case (state_q)
            ...
            READ: begin
                // Present the word captured at the preceding clock edge.
                // In parallel, put the next burst address on the asynchronous
                // SRAM so it has a full sys_clk period to settle.
                s_rvalid = 1'b1;
                s_rid    = ax_req_q_id;
                    ...
                    case (ax_req_q_burst)
                        ...
                        WRAP: begin
                            if (cons_addr == upper_wrap_boundary)
                                ...
                                addr_o = ax_req_q_addr +
                                         ((cnt_q - ax_req_q_len) << LOG_NR_BYTES);
                            else
                                addr_o = cons_addr;
                        end
                    endcase
    ...
    always @(posedge clk or negedge resetn) begin
        if (~resetn) begin
            ...
            // The SRAM is asynchronous.  The first address is driven in IDLE;
            // subsequent addresses are driven while the previous AXI beat is
            // being consumed, allowing one read beat per sys_clk after startup.
            if ((state_q == IDLE && s_arvalid && s_arready) ||
                (state_q == READ && s_rready && !s_rlast))
                read_data_q <= data_i;
            state_q         <= state_d;
