...
自动 supervisor 场景会在 CPU 向 UART 写入开始标志 `0x06` 后清零并启动性能
计数，在写入结束标志 `0x07` 的同一周期停止并打印 `[PERF-*]` 汇总。计数窗口不
包含启动、命令传输和结束后的结果比较，不需要额外 plusarg。

- `cycles`/`instret` 与 `ipc`
- 分支数、错误预测数、准确率和错误恢复周期
- I/D Cache access、miss、MPKI 和 miss 活跃周期
- load-use、store、mul、div 和访存结构冲突的独立周期
- 每周期互斥的 `STALL_NONE/ICACHE/DCACHE/LOAD_USE/MULDIV/STORE/BRANCH/
  MEM_STRUCTURAL/OTHER` 归因
- D-Cache writeback 次数/周期、SRAM 读写 beat、uncached access

`[PERF-CHECK] result=PASS` 表示基本不变量和主停顿周期总和均通过检查。计数探针
只存在于 `verilator_tb` 和 C++ 驱动中，不修改 CPU 接口，也不会进入 FPGA 综合。

阶段二可在同一测试后追加 `+cpu5_dcache_bypass`，强制所有可缓存数据访问走
direct 单拍通路，用于与正常 D-Cache 做 A/B 对照。该开关只在 Verilator 定义
下有效，Vivado 综合时恒为关闭；`[PERF-DIRECT]` 会单独报告旁路等待周期。

阶段二、三的正式 RTL 使用一个 6-bit 全局 miss 压力计数器，只把高 miss 场景
的 miss 改成单字旁路；旁路 store 进入 4 项 Store Buffer。Buffer 支持
同地址合并、按字节覆盖的 load forwarding，普通 refill 和 CACOP 会等待其排空，
UART/MMIO 不会进入 Buffer。`[PERF-MEM]` 报告旁路、入队、前递、满周期和最大
占用。仅调试时可追加 `+cpu5_dcache_adaptive_eager`，把预测器复位为饱和状态，
用于用现有程序压测 Store Buffer；该开关同样不会进入 Vivado 综合。
...
- `[PERF-PIPE-DETAIL]`：分别统计 ID/EX 各级的 load 和 mul RAW 互锁周期。
- `[PERF-BP-DETAIL]`：统计预测 taken、实际 taken、正确 taken 仍被冲刷，以及
  BTB 未命中却预测 taken 的次数。

正常情况下 `correct_taken_flush` 和 `fetch_taken_btb_miss` 都应为 `0`。这些探针仅在
Verilator testbench 中统计，不进入 FPGA 综合。
