...
struct PerfCounters {
    ...
    uint64_t direct_wait_cycles = 0;
    uint64_t load_use_stall_cycles = 0;
    uint64_t store_stall_cycles = 0;
    uint64_t mul_stall_cycles = 0;
    uint64_t div_stall_cycles = 0;
    uint64_t mem_structural_stall_cycles = 0;
    uint64_t dcache_writebacks = 0;
    uint64_t dcache_writeback_cycles = 0;
    uint64_t dcache_adaptive_bypasses = 0;
    uint64_t dcache_adaptive_bypass_cycles = 0;
    uint64_t sram_read_beats = 0;
    uint64_t sram_write_beats = 0;
    uint64_t uncached_accesses = 0;
    uint64_t store_buffer_full_cycles = 0;
    uint64_t store_buffer_enqueues = 0;
    uint64_t store_buffer_forwards = 0;
    uint64_t store_buffer_max_occupancy = 0;
    uint64_t dcache_read_wait_store_cycles = 0;
    uint64_t id_load_ex1_stall_cycles = 0;
    uint64_t id_load_mem_stall_cycles = 0;
    uint64_t id_mul_ex1_stall_cycles = 0;
    uint64_t id_mul_ex2_stall_cycles = 0;
    uint64_t ex_load_ex2_stall_cycles = 0;
    uint64_t ex_load_exmem_stall_cycles = 0;
    uint64_t ex_load_mem1_stall_cycles = 0;
    uint64_t branch_pred_taken = 0;
    uint64_t branch_actual_taken = 0;
    uint64_t branch_correct_taken = 0;
    uint64_t branch_correct_taken_flush = 0;
    uint64_t fetch_pred_taken_btb_miss = 0;

    uint64_t stall_none = 0;
    uint64_t stall_icache = 0;
    uint64_t stall_dcache = 0;
    uint64_t stall_direct = 0;
    uint64_t stall_load_use = 0;
    uint64_t stall_muldiv = 0;
    uint64_t stall_store = 0;
    uint64_t stall_branch = 0;
    uint64_t stall_mem_structural = 0;
    uint64_t stall_other = 0;

    bool branch_recovery_active = false;
    bool dcache_miss_active = false;
    bool dcache_writeback_active_prev = false;
    bool dcache_adaptive_bypass_active_prev = false;
    bool dcache_forced_bypass = false;

    void reset() {
        *this = PerfCounters{};
    }

    void sample(const Vverilator_tb& top) {
        ++cycles;
        dcache_forced_bypass =
            dcache_forced_bypass || top.perf_dcache_forced_bypass;
        ...
        if (top.perf_branch_mispredict) {
            ++branch_mispredicts;
        }
        if (top.perf_icache_access) {
            ++icache_accesses;
        }
        if (top.perf_icache_miss) {
            ++icache_misses;
        }
        bool icache_miss_cycle =
            top.perf_icache_miss || top.perf_icache_miss_active;
        if (icache_miss_cycle) {
            ++icache_miss_cycles;
        }

        if (top.perf_dcache_access) {
            ++dcache_accesses;
        }
        if (top.perf_dcache_miss) {
            ++dcache_misses;
            dcache_miss_active = true;
        }
        bool dcache_miss_cycle = dcache_miss_active;
        if (dcache_miss_cycle) {
            ++dcache_miss_cycles;
            if (top.perf_dcache_complete) {
                dcache_miss_active = false;
            }
        }
        if (top.perf_direct_wait) {
            ++direct_wait_cycles;
        }

        if (top.perf_load_use_stall) {
            ++load_use_stall_cycles;
        }
        if (top.perf_store_stall) {
            ++store_stall_cycles;
        }
        if (top.perf_mul_stall) {
            ++mul_stall_cycles;
        }
        if (top.perf_div_stall) {
            ++div_stall_cycles;
        }
        if (top.perf_mem_structural_stall) {
            ++mem_structural_stall_cycles;
        }

        bool writeback_active = top.perf_dcache_writeback_active;
        if (writeback_active) {
            ++dcache_writeback_cycles;
            if (!dcache_writeback_active_prev) {
                ++dcache_writebacks;
            }
        }
        dcache_writeback_active_prev = writeback_active;

        bool adaptive_bypass_active =
            top.perf_dcache_adaptive_bypass;
        if (adaptive_bypass_active) {
            ++dcache_adaptive_bypass_cycles;
            if (!dcache_adaptive_bypass_active_prev) {
                ++dcache_adaptive_bypasses;
            }
        }
        dcache_adaptive_bypass_active_prev = adaptive_bypass_active;

        if (top.perf_sram_read_beat) {
            ++sram_read_beats;
        }
        if (top.perf_sram_write_beat) {
            ++sram_write_beats;
        }
        if (top.perf_uncached_access) {
            ++uncached_accesses;
        }
        if (top.perf_store_buffer_full) {
            ++store_buffer_full_cycles;
        }
        if (top.perf_store_buffer_enqueue) {
            ++store_buffer_enqueues;
        }
        if (top.perf_store_buffer_forward) {
            ++store_buffer_forwards;
        }
        if (top.perf_dcache_read_wait_store) {
            ++dcache_read_wait_store_cycles;
        }
        store_buffer_max_occupancy = std::max(
            store_buffer_max_occupancy,
            static_cast<uint64_t>(top.perf_store_buffer_count));
        if (top.perf_id_load_ex1_stall) {
            ++id_load_ex1_stall_cycles;
        }
        if (top.perf_id_load_mem_stall) {
            ++id_load_mem_stall_cycles;
        }
        if (top.perf_id_mul_ex1_stall) {
            ++id_mul_ex1_stall_cycles;
        }
        if (top.perf_id_mul_ex2_stall) {
            ++id_mul_ex2_stall_cycles;
        }
        if (top.perf_ex_load_ex2_stall) {
            ++ex_load_ex2_stall_cycles;
        }
        if (top.perf_ex_load_exmem_stall) {
            ++ex_load_exmem_stall_cycles;
        }
        if (top.perf_ex_load_mem1_stall) {
            ++ex_load_mem1_stall_cycles;
        }
        if (top.perf_branch_pred_taken) {
            ++branch_pred_taken;
        }
        if (top.perf_branch_actual_taken) {
            ++branch_actual_taken;
        }
        if (top.perf_branch_correct_taken) {
            ++branch_correct_taken;
        }
        if (top.perf_branch_correct_taken_flush) {
            ++branch_correct_taken_flush;
        }
        if (top.perf_fetch_pred_taken_btb_miss) {
            ++fetch_pred_taken_btb_miss;
        }

        bool branch_recovery_cycle =
            branch_recovery_active && !top.perf_ex_valid;
        if (branch_recovery_cycle) {
            ++branch_flush_cycles;
        }
        if (branch_recovery_active && top.perf_ex_valid) {
        ...
        // Mutually exclusive primary stall attribution.  A cycle is NONE when
        // the pipeline is making normal progress, even if no instruction has
        // reached retirement yet.  Detailed counters above remain independent.
        if (icache_miss_cycle) {
            ++stall_icache;
        } else if (dcache_miss_cycle) {
            ++stall_dcache;
        } else if (top.perf_load_use_stall) {
            ++stall_load_use;
        } else if (top.perf_mul_stall || top.perf_div_stall) {
            ++stall_muldiv;
        } else if (top.perf_store_stall) {
            ++stall_store;
        } else if (top.perf_dcache_wait) {
            ++stall_dcache;
        } else if (top.perf_direct_wait) {
            ++stall_direct;
        } else if (branch_recovery_cycle) {
            ++stall_branch;
        } else if (top.perf_mem_structural_stall) {
            ++stall_mem_structural;
        } else if (top.perf_pipeline_stall) {
            ++stall_other;
        } else {
            ++stall_none;
        }
    ...
    void print() const {
        ...
        double branch_accuracy =
            branches == 0
                ? 0.0
                : 100.0 *
                      (1.0 - ratio(branch_mispredicts, branches));
        uint64_t stall_sum =
            stall_none + stall_icache + stall_dcache + stall_direct +
            stall_load_use + stall_muldiv + stall_store + stall_branch +
            stall_mem_structural + stall_other;
        bool checks_ok =
            instret <= cycles &&
        ...
        std::cout << std::fixed << std::setprecision(6)
                  << "[PERF] cycles=" << cycles
                  << " instret=" << instret
                  << " ipc=" << ratio(instret, cycles) << "\n"
                  << "[PERF-BRANCH] branch=" << branches
                  << " mispredict=" << branch_mispredicts
                  << " accuracy_pct=" << branch_accuracy
                  << " flush_cycles=" << branch_flush_cycles << "\n"
                  << "[PERF-ICACHE] access=" << icache_accesses
                  << " miss=" << icache_misses
                  << " mpki=" << 1000.0 * ratio(icache_misses, instret)
                  << " miss_cycles=" << icache_miss_cycles << "\n"
                  << "[PERF-DCACHE] access=" << dcache_accesses
                  << " miss=" << dcache_misses
                  << " mpki=" << 1000.0 * ratio(dcache_misses, instret)
                  << " miss_cycles=" << dcache_miss_cycles << "\n"
                  << "[PERF-DIRECT] wait_cycles=" << direct_wait_cycles
                  << " forced_bypass=" << dcache_forced_bypass << "\n"
                  << "[PERF-STALL-DETAIL] load_use=" << load_use_stall_cycles
                  << " store=" << store_stall_cycles
                  << " mul=" << mul_stall_cycles
                  << " div=" << div_stall_cycles
                  << " mem_structural=" << mem_structural_stall_cycles
                  << "\n"
                  << "[PERF-STALL-PRIMARY] none=" << stall_none
                  << " icache=" << stall_icache
                  << " dcache=" << stall_dcache
                  << " direct=" << stall_direct
                  ...
                  << " store=" << stall_store
                  << " branch=" << stall_branch
                  << " mem_structural=" << stall_mem_structural
                  << " other=" << stall_other << "\n"
                  << "[PERF-MEM] dcache_writeback=" << dcache_writebacks
                  << " dcache_writeback_cycles="
                  << dcache_writeback_cycles
                  << " adaptive_bypass=" << dcache_adaptive_bypasses
                  << " adaptive_bypass_cycles="
                  << dcache_adaptive_bypass_cycles
                  << " sram_read_beats=" << sram_read_beats
                  << " sram_write_beats=" << sram_write_beats
                  << " uncached_access=" << uncached_accesses
                  << " store_buffer_full_cycles="
                  << store_buffer_full_cycles
                  << " store_buffer_enqueues=" << store_buffer_enqueues
                  << " store_buffer_forwards=" << store_buffer_forwards
                  << " store_buffer_max_occupancy="
                  << store_buffer_max_occupancy
                  << " read_wait_store_cycles="
                  << dcache_read_wait_store_cycles << "\n"
                  << "[PERF-PIPE-DETAIL] id_load_ex1="
                  << id_load_ex1_stall_cycles
                  << " id_load_mem=" << id_load_mem_stall_cycles
                  << " id_mul_ex1=" << id_mul_ex1_stall_cycles
                  << " id_mul_ex2=" << id_mul_ex2_stall_cycles
                  << " ex_load_ex2=" << ex_load_ex2_stall_cycles
                  << " ex_load_exmem=" << ex_load_exmem_stall_cycles
                  << " ex_load_mem1=" << ex_load_mem1_stall_cycles
                  << "\n"
                  << "[PERF-BP-DETAIL] pred_taken=" << branch_pred_taken
                  << " actual_taken=" << branch_actual_taken
                  << " correct_taken=" << branch_correct_taken
                  << " correct_taken_flush="
                  << branch_correct_taken_flush
                  << " fetch_taken_btb_miss="
                  << fetch_pred_taken_btb_miss << "\n"
                  << "[PERF-CHECK] instret_le_cycles="
                  << (instret <= cycles)
                  << " branch=" << (branch_mispredicts <= branches)
                  << " icache=" << (icache_misses <= icache_accesses)
                  << " dcache=" << (dcache_misses <= dcache_accesses)
                  << " stall_sum=" << stall_sum
                  << " result=" << (checks_ok ? "PASS" : "FAIL")
                  << std::defaultfloat << "\n"
                  << std::flush;
    }
};
...
int main(int argc, char** argv) {
    ...
    while (!Verilated::gotFinish() && (term_mode || main_time < max_time)) {
        ...
        if (clk_posedge) {
            bool perf_start =
                top.uart_display && top.uart_data == 0x06U;
            bool perf_end =
                top.uart_display && top.uart_data == 0x07U;
            if (perf_start) {
                perf.reset();
                perf_active = true;
            } else if (perf_end) {
